          ...
          <div className="flex 2xl:gap-4 gap-3 lg:flex-row flex-col">
            <div className="lg:w-3/5 flex flex-col 2xl:gap-4 gap-3">
              ...
              <div className="grid grid-cols-2 2xl:gap-4 gap-3">
                ...
                <Input type="text" name="invoiceNo" label="Invoice No" defaultValue={data?.invoiceNo} disabled />
              </div>
              <div className="grid grid-cols-2 2xl:gap-4 gap-3">
                <Input type="text" name="lotNo" label="Lot No" defaultValue={data?.lotNo} disabled />
                <Input type="date" name="expiryDate" label="Expiry Date" defaultValue={format(new Date(data?.expiryDate), 'yyyy-MM-dd')} disabled />
              </div>
            </div>
            <div className="lg:w-2/5 2xl:gap-4 gap-3 lg:border-s lg:ps-4">
              <div className="grid lg:gap-52 gap-16">
                <div className="grid  gap-3">
                  ...
                  <Input type="number" name="quantity" label="Product quantity" inline className="lg:w-2/3" labelClassName="lg:w-1/3" value={data?.quantity} disabled />
                  <Input type="number" name="price" label="Unit price" inline className="lg:w-2/3" labelClassName="lg:w-1/3" value={data?.price} disabled />
                </div>
                    ...
                    <div className="grid  space-y-1 text-right items-center">
                      ...
                      <div className="relative ml-auto w-2/3">
                        <Input
                          ...
                          type="number"
                          step="0.001"
                          className={`pr-6 text-lg ${data?.due - amount < 0 ? 'border-red-500' : 'border-blue-300'}`}
                          required
                      ...
                      <p className={`font-bold ${data?.due - amount < 0 ? 'text-red-500' : 'text-textPrimary'}`}>
                        {data?.due - amount}
                        TK
                      </p>
                    </div>

...
export default function ProductTable({ products }: { products: any[] }) {
  ...
  const columns = [
    { key: "code", label: "Code" },
    {
      key: "category",
      label: "Category",
      render: (row: any) => <div className="capitalize">{row?.category[0]?.name}</div>,
    },
    ...
    },
    {
      key: "shelve",
      label: "Shelve",
      render: (row: any) => <div className="capitalize">{row?.shelve[0]?.name}</div>,
    },
    { key: "unit", label: "Unit" },
    { key: "price", label: "Price" },
    { key: "discountPercent", label: "Discount %" },
    { key: "discountAmount", label: "Discount", render: (row: any) => <div className="capitalize">{row?.discountAmount}</div>, },
    {
      key: "createdAt",
      label: "Entry On",
      customClass: "pl-8",

...
export default function SalesReportPaymentWiseTable({
  payments,
  startDate,
  endDate,
  totalPages,
  ...
  const columns = [
      ...
      render: (row: any) => {
        return (
          <button
            ...
            onClick={() => handlePrintInvoice(row?.sell)}
          >
            {row?.sell}
          </button>
        );
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="capitalize text-right font-medium">{toFixedIfNecessary(row?.discountAmount, 2)}</div>
      ),
    },
    {
      key: "amount",
    ...
    <div>
      <div className="flex gap-5 lg:flex-row flex-col">
        ...
        <div className="px-4 pb-6 w-full card">
          {/* Table component */}
          <Table
            title="Sales report (Payment wise)"
            caption={
              <div className="pt-3">
                ...
                <h3 className="test-base font-bold text-black mb-2 print:mb-0 pt-2 px-2 print:border-gray-600 border-b-4 border-double capitalize flex justify-between">
                  <span>{`Payment received By: ${createdBy}`}</span>
            ...
            print
            pageStyle={pageStyle}
            sumFields={["amount", "discountAmount"]}
            backBtn

...
export default function ExpenseTable({
    ...
    // filter start
    const dataGroupByEntryBy = GroupByHelper.groupBy(expenses, ({ createdBy }: { createdBy: any }) => createdBy?.name);

    const filteredData = data
        .filter((item: any) => createdBy === 'All' || item?.createdBy?.name === createdBy)
    // filter end
    ...
    const columns = [
        ...
        { key: "description", label: "Description", render: (row: any) => <div className="line-clamp-3 max-w-72">{row?.description}</div> },
        ...
        },
        {
            ...
            render: (row: any) => <div className="capitalize text-right font-medium">{row?.amount}</div>,
        },
    ];
            ...
            <div className="flex space-x-5">
                <div className="w-96">
                    <div className="sticky 2xl:top-[70px] top-[60px] p-4 card max-h-[85vh] overflow-auto">
                        ...
                        <button className={`py-0.5 px-2 text-sm border block w-full text-left whitespace-nowrap ${createdBy === 'All' ? 'bg-primary bg-opacity-10 text-primary' : 'focus:bg-primary focus:bg-opacity-10 focus:text-primary hover:bg-gray-200'
                            }`} onClick={() => setCreatedBy('All')}>
                            All
                        </button>
                        {Object.entries(dataGroupByEntryBy).map(([key, value]) => (
                            <button className={`py-0.5 px-2 text-sm border block w-full text-left whitespace-nowrap capitalize ${createdBy === key ? 'bg-primary bg-opacity-10 text-primary' : 'focus:bg-primary focus:bg-opacity-10 focus:text-primary hover:bg-gray-200'
                                }`} key={key} onClick={() => setCreatedBy(key)}>
                                {key}
                            </button>
                        ))}
                    </div>
                ...
                <div className="px-4 pb-6 w-full card">
                    ...
                    <Table
                        ...
                        caption={
                            <div className="pt-3">
                                <h2 className="hidden print:block text-black text-xl font-bold underline">Expense</h2>
